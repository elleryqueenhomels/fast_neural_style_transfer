...
def conv2d(x, input_filters, output_filters, kernel_size, strides, relu=True, mode='REFLECT'):
    ...
    weight = tf.Variable(tf.random.truncated_normal(shape, stddev=WEIGHT_INIT_STDDEV), name='weight')
...
def instance_norm(x, num_filters):
    ...
    x_normed  = tf.compat.v1.div(tf.subtract(x, mean), tf.sqrt(tf.add(var, epsilon)))
...
def transform(image):
    ...
    with tf.compat.v1.variable_scope('residual1'):
        res1 = residual(conv3, 128, 3, 1)
    with tf.compat.v1.variable_scope('residual2'):
        res2 = residual(res1, 128, 3, 1)
    with tf.compat.v1.variable_scope('residual3'):
        res3 = residual(res2, 128, 3, 1)
    with tf.compat.v1.variable_scope('residual4'):
        res4 = residual(res3, 128, 3, 1)
    with tf.compat.v1.variable_scope('residual5'):
    ...
    with tf.compat.v1.variable_scope('convout'):
