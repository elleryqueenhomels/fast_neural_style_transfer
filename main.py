...
if __name__ == '__main__':
	...
	model_save_path = 'models/starry.ckpt-done'

	if is_training:
		content_targets = list_images('images/content')
		print('\ncontent_targets:\n', content_targets, '\n')

		train(content_targets, style_target, content_weight, style_weight, tv_weight, save_path=model_save_path, batch_size=2, debug=True)
		...
		if condition == 1:
			content_targets = list_images('images/content')
			generated_images = generate(content_targets, model_path, resize_height=300, resize_width=300, save_path=output_save_path, postfix='-starry')
		elif condition == 2:
			content_targets = list_images('images/content')
